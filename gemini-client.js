(function () {
  ...
  async function simplifyPageWithGemini({ persona, domSummary, apiKey }) {
    const cleanApiKey = String(apiKey || "").trim();

    if (!cleanApiKey) {
      throw new Error(
        "Missing Gemini API key. Add extension/config.local.js or paste a key in PersonaLens."
      );
    }

    if (!persona || !domSummary?.elements) {
      throw new Error("Expected persona and extracted page elements before calling Gemini.");
    }

    const compactDom = compactDomSummary(domSummary);
    const personaStrategy = inferPersonaStrategy(persona);
    const prompt = buildPrompt({
      persona,
      domSummary: compactDom,
      personaStrategy
    });
    ...
    return validateAndNormalize(parsed, compactDom, personaStrategy);
  }

  function compactDomSummary(domSummary) {
    return {
      url: domSummary.url,
  ...
  async function getGeminiErrorMessage(response) {
    ...
    if (response.status === 400) return `Gemini request was rejected${suffix}`;
    if (response.status === 401 || response.status === 403) {
      return `Gemini API key was rejected${suffix}`;
    }
    if (response.status === 429) return `Gemini quota or rate limit was reached${suffix}`;

    return `Gemini API error (${response.status})${suffix}`;
  }
...
`;
  ...
  function validateAndNormalize(parsed, domSummary, personaStrategy) {
    ...
    return {
      mode: "reconstruct",
      explanation: String(
        parsed.explanation || "Rebuilt the page as a simpler version using selected original elements."
      ).slice(0, 320),
    ...
    for (const readable of pickReadableSupport(domSummary.elements, primaryElement, personaStrategy.readableLimit)) {
      ...
      addElement(result, usedIds, {
        ...
        visual: getDefaultVisual(
          readable.tag?.startsWith("h") ? "heading" : isProtectedElement(readable) ? "notice" : "text",
          readable,
          personaStrategy
        )
      });
  ...
  function enforcePersonaVisual(visual, purpose, personaStrategy) {
    ...
    if (personaStrategy.profile === "elderly_accessible") {
      ...
      visual.maxWidth = Math.max(
        500,
        Math.min(visual.maxWidth || defaultVisual.maxWidth || 700, defaultVisual.maxWidth || 700)
      );
    ...
    if (personaStrategy.profile === "student_contextual") {
      visual.fontScale = Math.max(
        0.95,
        Math.min(visual.fontScale || 1, Math.max(defaultVisual.fontScale || 1, 1.12))
      );
      visual.lineHeight = Math.max(
        1.38,
        Math.min(visual.lineHeight || 1.5, Math.max(defaultVisual.lineHeight || 1.45, 1.58))
      );
  ...
  window.PersonaLensGemini = {
    simplifyPageWithGemini
  };
})();
