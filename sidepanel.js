...
const enforceSameTabButton = document.getElementById("enforceSameTabButton");
...
enforceSameTabButton.addEventListener("click", enforceSameTabOnActiveTab);
...
function setLoading(isLoading, message = "Loading...") {
  ...
  statusPanel.setAttribute("aria-busy", String(isLoading));
}
...
async function showModelRequestLoading(message) {
...
async function buildPageAnalysisRequest() {
  if (!latestSnapshot) {
  ...
  try {
    await showModelRequestLoading("Preparing Gemini/OpenAI Page Analysis request...");
...
async function buildSimplificationStrategyRequest() {
  if (!latestSnapshot) {
  ...
  try {
    await showModelRequestLoading("Preparing Gemini/OpenAI Strategy request...");
...
async function buildStagedPatchPlanRequest() {
  if (!latestSnapshot) {
  ...
  try {
    await showModelRequestLoading("Preparing Gemini/OpenAI Patch Plan request...");
...
async function buildGeminiPatchRequest() {
  if (!latestSnapshot) {
  ...
  try {
    await showModelRequestLoading("Preparing Gemini/OpenAI one-shot request...");
...
async function applyPatchPlanToActiveTab(plan) {
  ...
  const sameTabResult = await enforceSameTabForTab(tab.id);
  const skippedCount = result.skipped?.length || 0;
  const skipped = skippedCount ? ` Skipped ${skippedCount} item(s): ${result.skipped[0]}` : "";
  const cssCount = extensionCss ? 1 : 0;
  const sameTabSummary = sameTabResult ? ` Same-tab: ${sameTabResult.linksUpdated || 0} link(s), ${sameTabResult.formsUpdated || 0} form(s).` : "";
  setStatus(`Patch applied: ${(result.applied || 0) + cssCount} change(s).${sameTabSummary}${skipped}`);
}

async function enforceSameTabOnActiveTab() {
  try {
    const tab = await getActiveTab();
    const result = await enforceSameTabForTab(tab.id);
    setStatus(`Same-tab behavior applied: ${result.linksUpdated || 0} link(s), ${result.formsUpdated || 0} form(s).`);
  } catch (error) {
    setStatus(error.message || "Same-tab behavior could not be applied.", true);
  }
}

async function enforceSameTabForTab(tabId) {
  const [{ result }] = await chrome.scripting.executeScript({
    target: { tabId },
    func: enforceSameTabInPage
  });
  if (!result) {
    throw new Error("No same-tab result returned from the page.");
  }
  return result;
}
...
function enforceSameTabInPage() {
  const PATCH_ATTR = "data-bridge-same-tab";
  const result = {
    linksUpdated: 0,
    formsUpdated: 0,
  ...
  normalizeCurrentLinksAndForms();
  patchWindowOpen();
  installSameTabClickInterceptor();
  installSameTabMutationObserver();
  return result;

  function normalizeCurrentLinksAndForms() {
    for (const link of document.querySelectorAll("a[href]")) {
      if (normalizeAnchor(link)) result.linksUpdated += 1;
    }

    for (const form of document.querySelectorAll("form[target]")) {
      const target = form.getAttribute("target");
      if (target && target !== "_self") {
        form.setAttribute("target", "_self");
        form.setAttribute(PATCH_ATTR, "true");
  ...
  function normalizeAnchor(link) {
    ...
    if (rel && /\bnoopener\b|\bnoreferrer\b/i.test(rel)) {
      const nextRel = rel
        .split(/\s+/)
    ...
    if (changed) {
      link.setAttribute(PATCH_ATTR, "true");
    }
    return changed;
  }

  function patchWindowOpen() {
    if (window.__bridgeSameTabWindowOpenPatched) return;
    const originalOpen = window.open;
    Object.defineProperty(window, "__bridgeSameTabOriginalOpen", {
      value: originalOpen,
      configurable: true
    });
    window.open = function bridgeSameTabOpen(url) {
      if (url) {
        location.href = String(url);
      }
      return window;
    };
    window.__bridgeSameTabWindowOpenPatched = true;
    result.windowOpenPatched = true;
  }

  function installSameTabClickInterceptor() {
    if (window.__bridgeSameTabClickInterceptorInstalled) return;
    document.addEventListener("click", (event) => {
      const link = event.target?.closest?.("a[href]");
      if (!link || !shouldNavigateInSameTab(event, link)) return;

      normalizeAnchor(link);
      event.preventDefault();
      event.stopPropagation();
      location.href = link.href;
    }, true);
    window.__bridgeSameTabClickInterceptorInstalled = true;
    result.listenerInstalled = true;
  }

  function installSameTabMutationObserver() {
    if (window.__bridgeSameTabMutationObserver) return;
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
          if (!(node instanceof Element)) continue;
          if (node.matches?.("a[href]")) normalizeAnchor(node);
          for (const link of node.querySelectorAll?.("a[href]") || []) normalizeAnchor(link);
          if (node.matches?.("form[target]") && node.getAttribute("target") !== "_self") {
            node.setAttribute("target", "_self");
            node.setAttribute(PATCH_ATTR, "true");
          }
          for (const form of node.querySelectorAll?.("form[target]") || []) {
    ...
    window.__bridgeSameTabMutationObserver = observer;
  }

  function shouldNavigateInSameTab(event, link) {
    if (!link.href || link.href.startsWith("javascript:")) return false;
    if (link.getAttribute("download") != null) return false;
    if (event.defaultPrevented) return false;
    return event.metaKey || event.ctrlKey || event.shiftKey || event.altKey || event.button === 1 || Boolean(link.target && link.target !== "_self");
  }
}
